...
export class QuestionBankComponent implements OnInit {
  ...
  async submit() {
    if (this.questionPattern.difficultyLevel == "Hard") {
  ...
  async fromApi() {
    const arg = await this.service.getQuestionsFromApi(this.questionPattern.difficultyLevel.toLocaleLowerCase())
  ...
  apiFormpattern(arg, rand) {
    arg.results[0].incorrect_answers.splice(rand, 0, arg.results[0].correct_answer)
